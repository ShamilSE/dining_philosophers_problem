...
void	sem_initialize(t_general *general)
{
	...
	sem_unlink("pair");
	sem_unlink("full");
	general->full = sem_open("full", O_CREAT, 0777, 1);
	general->time = sem_open("time", O_CREAT, 0777, 1);
	general->death = sem_open("death", O_CREAT, 0777, 1);
	sem_wait(general->full);
...
t_data	*init_data(char **av)
{
	...
	data->pid = malloc(sizeof(pid_t) * general->philo_num);
